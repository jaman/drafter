defmodule Drafter.CharacterWidth.Default do
  @behaviour Drafter.CharacterWidth
  ...
  def grapheme(<<cp::utf8>>), do: codepoint(cp)

  def grapheme(<<cp::utf8, rest::binary>>) do
    case cluster_presentation(rest) do
      :emoji -> 2
      :text -> 1
      :none -> codepoint(cp)
    end
  ...
  def string(text) when is_binary(text) do
    if printable_ascii?(text) do
      byte_size(text)
    else
      text |> String.graphemes() |> Enum.reduce(0, &(&2 + grapheme(&1)))
    end
  end
  ...
  def printable_ascii?(<<byte, rest::binary>>) when byte >= 0x20 and byte < 0x7F do
  ...
  @doc "Display width in columns of a single codepoint."
  @spec codepoint(non_neg_integer()) :: non_neg_integer()
  def codepoint(cp) when cp >= 0x20 and cp < 0x7F, do: 1

  def codepoint(cp) do
    cond do
      in_table?(@zero_width, cp) -> 0
      in_table?(@wide, cp) -> 2
      true -> 1
    end
  end

  defp cluster_presentation(<<@variation_emoji::utf8, _::binary>>), do: :emoji
  defp cluster_presentation(<<@variation_text::utf8, _::binary>>), do: :text
  defp cluster_presentation(<<@zwj::utf8, _::binary>>), do: :emoji
  defp cluster_presentation(<<_::utf8, rest::binary>>), do: cluster_presentation(rest)
  defp cluster_presentation(_), do: :none

  defp in_table?(table, cp), do: search(table, cp, 0, tuple_size(table) - 1)

  defp search(_table, _cp, low, high) when low > high, do: false

  defp search(table, cp, low, high) do
    mid = div(low + high, 2)
    {range_low, range_high} = elem(table, mid)

    cond do
      cp < range_low -> search(table, cp, low, mid - 1)
      cp > range_high -> search(table, cp, mid + 1, high)
      true -> true
    end
  end
end
