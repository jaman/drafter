defmodule Drafter.TestApps.Counter do
  @moduledoc """
     ...
     %{
       children: [
         ...
         {Drafter.Widget.Label,
          %{
            text:
              "Controls: Space/+/→/↑ = increment, -/←/↓ = decrement, Enter = reset, q/Esc = quit",
            style: %{dim: true},
            align: :center
          }}
       ],
  ...
  def handle_event(:increment, state) do
    new_state = %{state | counter: state.counter + 1, message: "Incremented!"}
    {:ok, new_state}
  end

  def handle_event(:decrement, state) do
    new_state = %{state | counter: state.counter - 1, message: "Decremented!"}
    {:ok, new_state}
  end
  ...
  def handle_event({:key, :enter}, state) do
    new_state = %{state | counter: 0, message: "Counter reset!"}
    {:ok, new_state}
  end
